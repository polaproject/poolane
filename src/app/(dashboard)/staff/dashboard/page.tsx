    ...
    <div className="min-h-screen bg-paper pb-12">
      <div className="bg-ink text-paper px-5 sm:px-8 pt-8 pb-12 relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto">
          <p className="eyebrow text-paper/55 mb-2">Trợ lý · Poolane</p>
          <h1 className="font-heading text-4xl sm:text-5xl italic leading-tight">Xin chào, {user.fullName}</h1>
          <p className="text-sm text-paper/65 mt-3">Duyệt đăng ký, theo dõi học viên, hỗ trợ lớp học hàng ngày.</p>
        </div>
      </div>
          ...
          <div className="rounded-card-lg bg-white shadow-soft ring-1 ring-ink/8 overflow-hidden">
            <div className="px-5 py-4 border-b border-ink/8 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <p className="eyebrow text-ink/55">Đăng ký mới</p>
              </div>
                ...
                {recentRegistrations.map(r => (
                  ...
                  >
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-ink truncate">{r.student.user.fullName}</p>
                      <p className="text-xs text-ink/55 mt-0.5">
                        {format(r.session.date, 'EEE dd/MM', { locale: vi })} · {r.session.timeSlot === 'morning' ? 'sáng' : 'chiều'}
                      </p>
                    </div>
                    <ArrowRight className="h-4 w-4 text-ink/30 group-hover:translate-x-0.5 group-hover:text-accent transition" strokeWidth={2.25} />
                  </Link>
                ))}
              </div>
            )}
        ...
        {upcomingSessions.length > 0 && (
          <div className="rounded-card-lg bg-white shadow-soft ring-1 ring-ink/8 overflow-hidden">
            <div className="px-5 py-4 border-b border-ink/8 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Calendar className="h-4 w-4 text-accent" strokeWidth={1.75} />
                <p className="eyebrow text-ink/55">Tuần này tiếp theo</p>
              </div>
            ...
            <div className="grid grid-cols-5 divide-x divide-ink/5">
              {upcomingSessions.map(s => (
                ...
                >
                  <p className="text-[10px] tracking-widest uppercase text-ink/45">{format(s.date, 'EEE', { locale: vi })}</p>
                  <p className="font-heading italic text-2xl text-ink mt-0.5">{format(s.date, 'dd/MM')}</p>
                  <p className="text-xs text-ink/55 mt-1 inline-flex items-center gap-1">
                    {s.timeSlot === 'morning'
  ...
  return (
    <Link
      ...
      className={`group flex items-center gap-3 px-3 py-2.5 rounded-card transition ${
        primary ? 'bg-ink text-paper hover:bg-ink/90 shadow-soft' : 'hover:bg-paper-tint/50'
      }`}
    >
