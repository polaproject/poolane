...
export function ExerciseForm({ mode, initial }: { mode: 'create' | 'edit'; initial?: Partial<FormData> & { id?: string } }) {
  ...
  async function onDelete() {
    ...
    if (!confirm('Xoá bài tập này? Tất cả assignment sẽ bị xoá theo.')) return
    setSubmitting(true)
    try {
      const res = await fetch(`/api/exercises/${initial.id}`, { method: 'DELETE' })
      if (!res.ok) {
        const j = await res.json()
        setError(j.error?.message ?? 'Lỗi')
        setSubmitting(false)
        return
      }
      router.push('/admin/exercises')
  ...
  return (
    <form onSubmit={onSubmit} className="bg-white rounded-2xl shadow-sm border border-[#1C2B4A]/8 p-5 space-y-4">
        ...
        <div className="space-y-2">
          {form.steps.map((step, i) => (
            <div key={i} className="flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-[#1C2B4A]/10 text-[#1C2B4A] text-xs font-bold flex items-center justify-center flex-shrink-0">{i + 1}</span>
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        ...
        <span className="text-sm text-[#1C2B4A]">Đăng (học viên xem được trong thư viện)</span>
      </label>
      ...
      <div className="flex gap-3">
        ...
        <Link href="/admin/exercises" className="px-4 py-2.5 text-sm font-semibold rounded-lg border border-[#1C2B4A]/15 text-[#1C2B4A]/70">
          Huỷ
        </Link>
      </div>
    </form>
