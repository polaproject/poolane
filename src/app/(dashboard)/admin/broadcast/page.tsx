...
export default function BroadcastPage() {
  ...
  const inputClass = 'w-full h-11 px-4 text-sm rounded-pill bg-paper-tint/40 ring-1 ring-ink/10 focus:ring-accent/40 focus:outline-none transition'
  ...
  return (
    <div className="min-h-screen bg-paper pb-12">
      ...
      <div className="px-4 sm:px-8 -mt-6 max-w-2xl mx-auto space-y-4 relative z-10">
        {/* Templates */}
        <div className="rounded-card-lg bg-white shadow-soft ring-1 ring-ink/8 p-4">
          <p className="eyebrow text-ink/55 mb-2">Mẫu nhanh</p>
          <div className="flex gap-2 flex-wrap">
            {TEMPLATES.map(t => (
              <button
                key={t.label}
                onClick={() => { setTitle(t.title); setBody(t.body) }}
                className="text-xs px-3 py-1.5 rounded-pill bg-paper-tint/60 ring-1 ring-ink/10 text-ink hover:bg-paper-tint hover:ring-accent/40 transition"
              >
                {t.label}
              </button>
            ))}
        ...
        <form onSubmit={handleSend} className="space-y-4">
          ...
          <div className="rounded-card-lg bg-white shadow-soft ring-1 ring-ink/8 p-5">
            <p className="eyebrow text-ink/55 mb-3">Gửi đến</p>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-card text-left transition ring-1 ${
                      active
                        ...
                        : 'bg-paper-tint/30 ring-ink/10 hover:ring-accent/30'
                    }`}
                  >
                    <Icon className={`h-4 w-4 mb-2 ${active ? 'text-accent' : 'text-ink/60'}`} strokeWidth={1.75} />
                    <p className={`font-medium text-sm ${active ? 'text-paper' : 'text-ink'}`}>{opt.label}</p>
                    <p className={`text-xs mt-1 ${active ? 'text-paper/65' : 'text-ink/55'}`}>{opt.desc}</p>
                  </button>
                )
              })}
            </div>
          ...
          <div className="rounded-card-lg bg-white shadow-soft ring-1 ring-ink/8 p-5 space-y-4">
            <div>
              <label htmlFor="title" className="eyebrow text-ink/55 mb-1.5 block">
            ...
            <div>
              <label htmlFor="body" className="eyebrow text-ink/55 mb-1.5 block">
              ...
              <textarea
                ...
                rows={4}
                required
                className="w-full px-4 py-3 text-sm rounded-card-lg bg-paper-tint/40 ring-1 ring-ink/10 focus:ring-accent/40 focus:outline-none transition resize-none"
              />
            </div>

            {(title || body) && (
              <div className="rounded-card bg-paper-tint/40 ring-1 ring-ink/8 p-3">
                <p className="eyebrow text-ink/45 mb-2">Xem trước</p>
                <div className="flex gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-ink/65 mt-0.5">{body || '...'}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <button
            ...
            className="w-full inline-flex items-center justify-center gap-2 bg-ink text-paper font-semibold h-12 rounded-pill hover:bg-ink/90 transition disabled:opacity-60 shadow-soft"
          >
            {loading
