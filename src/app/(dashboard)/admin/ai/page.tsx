          ...
          <div className="space-y-3">
            {filtered.map(s => {
              const ringClass = s.riskLevel === 'high' ? 'ring-danger/30' :
                s.riskLevel === 'medium' ? 'ring-warn/30' : 'ring-foreground/8'
              const scoreClass = s.riskScore >= 50 ? 'bg-danger' :
                s.riskScore >= 25 ? 'bg-warn' : 'bg-success'
              return (
                <div key={s.studentId} className={`rounded-card-lg bg-[var(--surface)] shadow-soft ring-1 p-5 ${ringClass}`}>
                  <div className="flex justify-between items-start gap-3 mb-3">
                    <div>
                      <Link href={`/admin/students/${s.studentId}`} className="text-sm font-medium text-foreground hover:text-accent transition">
                        {s.fullName}
                      </Link>
                      <p className="text-xs text-foreground/55">{s.phone}</p>
                    </div>
                    <div className="text-right">
                      <div className={`text-paper text-sm font-bold px-3 py-1.5 rounded-pill ${scoreClass}`}>
                        {s.riskScore}%
                      </div>
                      <p className="text-xs text-foreground/45 mt-1">nguy cơ</p>
                    </div>
                  </div>
                  ...
                  <div className={`flex items-start gap-2 text-sm rounded-card px-3 py-2 ${
                    ...
                    <span className="text-foreground/85">{s.suggestedAction}</span>
                  </div>
                </div>
              )
            })}
