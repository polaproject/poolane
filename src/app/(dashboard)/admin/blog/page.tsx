              ...
              <table className="w-full min-w-[700px]">
                <thead className="bg-paper-tint/30 border-b border-foreground/8">
                  <tr>
                    ...
                    <th className="text-left px-5 py-3 eyebrow text-foreground/55">Danh mục</th>
                    <th className="text-left px-5 py-3 eyebrow text-foreground/55">Trạng thái</th>
                    <th className="text-right px-5 py-3 eyebrow text-foreground/55">Lượt xem</th>
                    <th className="text-left px-5 py-3 eyebrow text-foreground/55">Đăng</th>
                ...
                <tbody>
                  {posts.map(p => {
                    ...
                    return (
                      <tr key={p.id} className="border-b border-foreground/5 last:border-b-0 hover:bg-paper-tint/20 transition glass-table-row">
                          ...
                          <div className="flex items-center gap-3">
                            {p.coverImageUrl ? (
                              ...
                              <div className="w-12 h-12 rounded-card bg-paper-tint grid place-items-center shrink-0">
                                <FileText className="h-4 w-4 text-foreground/30" strokeWidth={1.75} />
                              </div>
                            )}
                            <div className="min-w-0">
                              <p className="font-medium text-sm text-foreground truncate">{p.title}</p>
                              <p className="text-xs text-foreground/45 font-mono mt-0.5">/{p.slug}</p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-3 text-right text-xs text-foreground/65">
                        ...
                        <td className="px-5 py-3 text-right">
                          <div className="flex gap-3 justify-end text-xs font-medium">
                            ...
                            <Link href={`/admin/blog/${p.id}/edit`} className="text-foreground/75 hover:text-accent transition">
                              Sửa
                            </Link>
                          </div>
                        </td>
